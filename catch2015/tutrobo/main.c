...
#if 1
	POWER_OUT=0;
	PSCON_PRE_L2=PSCON_PRE_R2=D_direction_R=D_direction_L=255;
//	TASK_START();         		// カウントスタート 割り込みスタート
//	SENCE_START();         		// カウントスタート 割り込みスタート
	...
	while(1)					// モーター電源ON
	{
		...
		if(HALF_1==255 && PSCON_PRE_L2==0 && PSCON_PRE_R2==0 && D_direction_R==0 && D_direction_L==0 && pscon_err_flag!=1)
		{
			POWER_OUT = ON;
			break;
		}
...
//		SERVO_LIMIT);
		...
		if(mode()=='R')printf("RED\t");
		else printf("BLUE\t");
		if(R_TOWER_UNDER_LIMIT==0)printf("R_TOWER_UNDER_LIMIT\t");
		if(L_TOWER_UNDER_LIMIT==0)printf("L_TOWER_UNDER_LIMIT\t");
		if(SERVO_LIMIT==0)printf("SERVO_LIMIT\t");
		printf("\n");
