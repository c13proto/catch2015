...
#if 1
/*********チャンネル6 pwmモード2設定開始 (サーボ)**********/


	MTUB.TSTR.BIT.CST0 = 0;     // MTU0カウントスタート
	
	MTU6.TCR.BIT.TPSC =2;     //クロック周波数φ/16 (MTUのクロックは48Mhz)
...
//	MTUB.TSYR.BYTE = 0;         //独立動作		
	...
	MTU6.TGRA = SERVO_TH;		// デューティ設定
	MTU6.TGRB = SERVO_TH;		// デューティ設定
	MTU6.TGRC = SERVO_TH;		// デューティ設定
	MTU6.TGRD = SERVO_CYCLE;		// 周期設定
			
	MTU6.TMDR.BIT.MD = 3;      // PWMモード2



/*****チャンネル9 pwmモード1設定開始 (サーボ)******/
	MTUB.TSTR.BIT.CST4 = 0; 	// MTU4カウント停止
	
	MTU9.TCR.BIT.CCLR = 2; 	// TGRBでカウントクリア;
	
	MTU9.TCR.BIT.TPSC = 2; 	//クロック周波数φ/16 (MTUのクロックは48Mhz)
	...
	MTU9.TGRA = SERVO_TH;		// TIOC4Aコンペアマッチ
	MTU9.TGRB = SERVO_CYCLE;	// TIOC4Bコンペアマッチ
	MTU9.TGRC = SERVO_TH;		// TIOC4Cコンペアマッチ
	MTU9.TGRD = SERVO_CYCLE;	// TIOC4Dコンペアマッチ
...
/*************チャンネル10 PWMモード1設定開始（サーボ）***************/
	...
	MTUB.TSTR.BIT.CST3 = 0; 	//MTU10カウント停止
...
MTUA.TSTR.BIT.CST4 = 1;     // MU24カウントスタート

MTUB.TSTR.BIT.CST0 = 1;     // MTU0カウントスタート
MTUB.TSTR.BIT.CST3 = 1;     //MTU4カウントスタート
MTUB.TSTR.BIT.CST4 = 1;     // MU24カウントスタート
